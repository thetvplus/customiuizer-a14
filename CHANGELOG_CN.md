...
调试验证版本，versionCode 214。面向 HyperOS 1 / Android 14。

- 恢复并保持 r14.21.5 的浅色、深色配色。
- 修正控制中心设置页的初始隐藏项处理时序；分组装饰读取实际适配器列表，避免过渡期间临时丢失间距。
- 恢复信号/Wi-Fi 左移，左侧容器沿用原生行高、内边距和图标高度；新增左侧图标大小（75%–125%）及垂直偏移（-6–6 dp）调节，修改后重启系统界面生效。
- 搜索覆盖独立设置页及开关项，支持页面路径和关键词组合检索、Wi-Fi/WIFI 同义匹配，结果直接定位对应控件；排除静态隐藏的不可用项。
- 开关行保留开关反馈与搜索高亮，去掉整行按压背景色。

实机视觉验收由用户执行，本版本不作为正式发布。
