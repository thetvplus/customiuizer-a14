...
versionCode 217。面向 HyperOS 1 / Android 14。

相较 r14.21.7：

- 优化应用锁和隐私应用列表的排序与搜索，减少重复状态查询和临时对象，保持已选优先、原有顺序与双开用户区分。
- 修复应用锁、隐私应用和快捷方式列表因图标缓存键未初始化而可能不加载图标的问题。
- 复用快捷方式名称和图标元数据，减少滚动与搜索中的重复资源读取。
- 优化设置搜索和应用列表的结果更新，减少不必要的列表复制。
