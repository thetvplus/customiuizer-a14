...
r14.21.* 只保留这一版。分组设置界面和通知频道路由一并保留，表面色改走 Telegram 设置页风格。

### 设置界面

- 偏好列表、搜索、应用 / Wi-Fi / 蓝牙选择、可排序动作、右上角菜单、对话框和关于页使用分组内嵌卡片。
- 统一 10 dp 圆角。分类标题为句首大写，不再使用全大写。首页分类图标不加。标题栏下方 1 dp 细线去掉。
- 只保留两套表面色，不再跟壁纸。浅色：`#EFEFF4` 页底 + 白卡片。深色：黑页底 + `#1C1C1D` 卡片。强调色为 Telegram 蓝（`#3390EC` / `#6CB7F9`），只用在开关和选中态。
- 分隔线为 Telegram 发丝（`#D9D9D9` / 8% 白）。
- 标题 / 摘要 / 分类字号为 16 / 13 / 14 sp。行高约 50 dp。
